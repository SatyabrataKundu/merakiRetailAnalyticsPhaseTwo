...
router.get("/historicalDataByCamera", function (req, res) {
    ...
    if (pattern == 'today') {
        ...
        +" FULL OUTER  JOIN (select * from meraki.visitor_predictions where dateformat_date ='" + date + "' and zoneid = 1) as  T2 "
...
function _datasetGeneration(dbInsertCamData) {
    return new Promise(function (fulfill, reject) {
        var insertQueryForDB = "INSERT INTO meraki.visitor_data_without_oid "
            + "(visitor_count,"
            + "datetime, "
            + "dateformat_date,"
            + "dateformat_year, "
        ...
        db.none(insertQueryForDB)
            .then(function (response) {
                console.log("db insert success for oid  ", dbInsertCamData.visitor_count);
                fulfill(response);
            })
            .catch(function (err) {
                console.log("not able to get connection " + err);
